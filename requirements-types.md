...
* **Актор:** Покупатель
* **Предусловие:** Покупатель находится в корзине с добавленными товарами.
* **Основной сценарий:**
...
1. **Смешение уровней требований:** Часто функциональные требования записывают в документ с бизнес-требованиями.
**Решение:** Четко разделяйте документы и артефакты для каждого уровня требований (BRD для бизнеса, User Stories для пользователей, спецификации для функциональных).

2. **Неоднозначность:** Требования вроде «система должна быть быстрой» или «интерфейс должен быть дружелюбным» бесполезны без конкретики.
**Решение:** Используйте измеримые критерии для нефункциональных требований (например, «время отклика < 1 секунды») и прототипы для согласования интерфейса.

3. **Отсутствие полноты:** Пропуск важных сценариев или состояний.
**Решение:** Проводите тщательный анализ, используйте техники, такие как диаграммы состояний, и привлекайте к обсуждению тестировщиков на ранних этапах.
