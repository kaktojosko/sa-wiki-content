...
```plantuml
@startuml
title OpenAPI спецификация: документация и кодогенерация

' ====== Style (современно и аккуратно) ======
!theme materia
...
skinparam ActivityStartColor #22C55E
...
start
:Написание спецификации OpenAPI\nв Swagger Editor;
:Спецификация OpenAPI\n(YAML/JSON);

' ====== Разводим стрелки влево/вправо без fork ======
if ("Артефакты") then ("Документация")
  :Генерация документации\nчерез **Swagger UI**;
  :Тестирование API\nчерез интерфейс **Swagger UI**;
else ("Кодогенерация")
  :Генерация клиентского SDK\nи серверных заглушек\nчерез **Swagger Codegen**;
  :Интеграция SDK и заглушек\nв клиентский и серверный код;
endif
...
1. **Неполное или неточное описание**: Одна из самых частых проблем — это неполное описание эндпоинтов, параметров или моделей данных. 
**Решение**: Следуйте принципу "документация как код" (documentation as code) и интегрируйте процесс обновления спецификации в ваш CI/CD пайплайн. Используйте линтеры для проверки полноты и корректности спецификации.
2. **Рассинхронизация между кодом и документацией**: Код меняется, а документация остается старой. 
**Решение**: Автоматизируйте генерацию спецификации из кода (code-first подход) с помощью аннотаций или, наоборот, генерируйте код из спецификации (design-first подход). Оба подхода помогают поддерживать документацию в актуальном состоянии.
3. **Слишком сложная или запутанная структура спецификации**: Большие и сложные API могут приводить к громоздким и трудным для чтения спецификациям. 
**Решение**: Используйте раздел `components` для переиспользования схем, параметров и ответов. Разбивайте большую спецификацию на несколько файлов и используйте `$ref` для их связывания.
