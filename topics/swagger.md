...
```mermaid
flowchart TD
  A[Написание спецификации OpenAPI в Swagger Editor] --> B{"Спецификация OpenAPI<br/>(YAML/JSON)"}
  B --> C[Генерация интерактивной документации<br/>через Swagger UI]
  B --> D[Генерация клиентского SDK и серверных заглушек<br/>через Swagger Codegen]
  C --> E[Тестирование API через интерфейс Swagger UI]
  D --> F[Интеграция SDK/заглушек<br/>в клиентский и серверный код]
```
